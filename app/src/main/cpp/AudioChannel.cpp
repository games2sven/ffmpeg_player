...
AudioChannel::AudioChannel(int id, AVCodecContext *avCodecContext,AVRational time_base) : BaseChannel(id, avCodecContext,time_base) {
    out_channels = av_get_channel_layout_nb_channels(AV_CH_LAYOUT_STEREO);//输出声道数
    out_samplesize = av_get_bytes_per_sample(AV_SAMPLE_FMT_S16);//输出采样大小
...
int AudioChannel::getPcm() {
    ...
    //可能播放的是48000HZ 8位的数据， 但是我们要按照通用格式 44100 16位的设置来播放，所以要将48000HZ=》44100
    //音频重采样
    // 假设我们输入了10个数据 ，swrContext转码器 这一次处理了8个数据
    // 那么如果不加delays(上次没处理完的数据) , 积压(栈内堆积)
    ...
    //获取 samples 个   * 2 声道 * 2字节（16位）
    data_size = samples * out_samplesize * out_channels ;
    //获得相对播放这一段数据的秒数(获取播放这一帧数据的秒数)
